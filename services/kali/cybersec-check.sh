...
# ---OpenSCAP---
# https://github.com/OVALProject/Test-Content/tree/master/linux
mkdir -p "$results_folder/oval"
# Iterate over each .xml file in the directory
for xml_file in "$kali_folder/oval"/*.xml; do
    if [ -f "$xml_file" ]; then
        # Extract the file name without extension
        filename=$(basename -- "$xml_file")
        filename_no_ext="${filename%.*}"

        # Execute the .xml file with oscap
        oscap oval eval --report "$results_folder/oval/$filename_no_ext-results.html" "$xml_file"
    fi
done
